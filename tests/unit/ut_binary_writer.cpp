...
TEST_F(binary_writer_test, write_span) {
    ...
    ASSERT_EQ(std::distance(writer.dw_begin(), writer.dw_end()), data_to_write.size_bytes());

    for (auto itr = writer.dw_begin(), ite = data_to_write.begin(); itr != writer.dw_end();
         itr++, ite++) {
        EXPECT_EQ(*itr, *ite);
    }
}
...
TEST_F(binary_writer_test, seek) {
    ...
    ASSERT_EQ(std::distance(writer.dw_begin(), writer.dw_end()), expected_offset);
    ...
    for (auto itr = writer.dw_begin(), ite = expected_bytes_vw.begin(); itr != writer.dw_end();
         itr++, ite++) {
        EXPECT_EQ(*itr, *ite);
    }
}
...
TEST_F(binary_writer_test, negative_advance) {
    ...
    constexpr tdsl::byte_view data_to_write{&source_buffer [0], &source_buffer [4]};
    ASSERT_TRUE(writer.write(data_to_write));
    ASSERT_EQ(std::distance(writer.dw_begin(), writer.dw_end()),
    ...
    for (auto itr = writer.dw_begin(), ite = data_to_write2.begin(); itr != writer.dw_end();
         itr++, ite++) {
        EXPECT_EQ(*itr, *ite);
    }
}
...
TEST_F(binary_writer_test, endianness_ne2be) {
    ...
    for (auto itr = writer.dw_begin(), ite = expected_vw.begin(); itr != writer.dw_end();
         itr++, ite++) {
        EXPECT_EQ(*itr, *ite);
    }
}
