...
namespace tdsl {
    ...
    namespace detail {
        ...
        struct login_context {
            ...
            /**
             * Login parameters (ASCII)
             */
            using login_parameters      = login_parameters_type<string_view>;

            /**
             * Login parameters (ASCII, PROGMEM)
             */
            using pmem_login_parameters = login_parameters_type<progmem_string_view>;
        ...
        public:
            ...
            static inline auto encode_password(tdsl::uint8_t * buf, tdsl::uint32_t sz) noexcept
                ...
                // ibChangePassword, the client SHOULD first swap" "the four high bits with the four
                // low bits and then do a bit-XOR with 0xA5 (10100101).""
            ...
            auto do_login(const LoginParamsType & params) noexcept -> e_login_status {
                ...
                for (pass_type pt = pass_type::begin; pt < pass_type::end;
                    ...
                    for (param_idx pi = param_idx::begin; pi < param_idx::end;
                        ...
                        switch (pi) {
                            ...
                            case param_idx::client_id:
                                if (pt == pass_type::offset_size_table) {
                                    tds_ctx.write(tdsl::span<>{&params.client_id [0],
                                                               sizeof(params.client_id)});
                                }
                                continue;
                                break;
                        ...
                        if (pt == pass_type::offset_size_table) {
                            ...
                            // 7 chars of UTF-8 is still 7 chars of ascii
                ...
                // TODO: More definitive login error codes
