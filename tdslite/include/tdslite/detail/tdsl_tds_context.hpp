...
namespace tdsl { namespace detail {
        ...
        private:
             ...
             * message data.
             */
            inline tdsl::uint32_t handle_tabular_result_msg(
                tdsl::binary_reader<tdsl::endian::little> & msg_rdr) noexcept {
                ...
                while (msg_rdr.has_bytes(/*amount_of_bytes=*/k_min_token_need_bytes)) {
                    ...
                    const auto token_type =
                        static_cast<e_token_type>(msg_rdr.read<tdsl::uint8_t>());
             ...
             */
            inline tdsl::uint32_t
            handle_info_token(tdsl::binary_reader<tdsl::endian::little> & rr) {
            ...
            handle_loginack_token(tdsl::binary_reader<tdsl::endian::little> & rr) {
                ...
                TDSL_DEBUG_PRINT(
                    "received login ack token -> interface [%d] | tds version [0x%x] | ",
             ...
             */
            inline tdsl::uint32_t
            handle_done_token(tdsl::binary_reader<tdsl::endian::little> & rr) {
                // fe
